...
public class Message0x05CoreUpdate extends SimpleChannelInboundHandler<Message0x05CoreUpdate>
    ...
    public Message0x05CoreUpdate(TileEntityBarrel barrel) {
        ...
        for (UpgradeCore i : barrel.coreUpgrades.upgradeList) upgrades.add(i);
    }
    ...
    public void encodeInto(ChannelHandlerContext ctx, IBarrelMessage msg, ByteBuf target) throws Exception {
        ...
        for (UpgradeCore i : upgrades) target.writeInt(i.ordinal());
    }
    ...
    public void decodeInto(ChannelHandlerContext ctx, ByteBuf dat, IBarrelMessage rawmsg) {
        ...
        for (int i = 0; i < size; i++) msg.upgrades.add(UpgradeCore.values()[dat.readInt()]);

        for (UpgradeCore i : msg.upgrades) {
            if (i.type == UpgradeCore.Type.STORAGE) {
                msg.nStorageUpg += i.slotsUsed;
            } else if (i == UpgradeCore.ENDER) msg.hasEnder = true;
            else if (i == UpgradeCore.HOPPER) msg.hasHopper = true;
            else if (i == UpgradeCore.REDSTONE) msg.hasRedstone = true;
            else if (i == UpgradeCore.VOID) msg.hasVoid = true;
            else if (i == UpgradeCore.CREATIVE) msg.hasCreative = true;
        }
    }
