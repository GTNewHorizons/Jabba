...
public class TileEntityBarrelRenderer extends TileEntityBaseRenderer {
    ...
    protected static ItemStack coreCreative = new ItemStack(
            BetterBarrels.itemUpgradeCore,
            0,
            UpgradeCore.CREATIVE.ordinal());

    public static TileEntityBarrelRenderer instance() {
        if (_instance == null) _instance = new TileEntityBarrelRenderer();
        return _instance;
    }
    ...
    public void renderTileEntityAt(TileEntity tileEntity, double xpos, double ypos, double zpos, float var8) {
        if (tileEntity instanceof TileEntityBarrel && BetterBarrels.renderStackAndText
                && (xpos + 0.5) * (xpos + 0.5) + (ypos + 0.5) * (ypos + 0.5) + (zpos + 0.5) * (zpos + 0.5)
                        < BetterBarrels.renderDistance) {
            this.saveBoundTexture();
            // int[][] savedGLState = modifyGLState(new int[]{ GL11.GL_BLEND }, new int[]{ GL11.GL_LIGHTING });
            // the following is how it was set, enabling lighting makes it look better with smooth lighting, mostly...
            // bottom labels are abit dark
            // will need to actually properly calculate lighting, but for now this looks better on smooth lit barrels
            // than the full bright it was before
            // found another bug: disabling lighting causes grass to look white, and a few other quirks, putting this on
            // hold for the momment
            ...
            boolean isHammer = this.mc.thePlayer.getHeldItem() != null
                    && this.mc.thePlayer.getHeldItem().getItem() instanceof IOverlayItem;
            ...
            for (ForgeDirection forgeSide : ForgeDirection.VALID_DIRECTIONS) {
                ...
                if (hasItem && this.isItemDisplaySide(barrelEntity, forgeSide)) {
                    ...
                    this.renderStackOnBlock(
                            barrelEntity.getStorage().getItemForRender(),
                            forgeSide,
                            isTopBottom ? rotation : orientation,
                            barrelPos,
                            8.0F,
                            65.0F,
                            isTopBottom ? 64.0F : 75.0F);
                    String barrelString = this.getBarrelString(barrelEntity);
                    this.renderTextOnBlock(
                            barrelString,
                            forgeSide,
                            isTopBottom ? rotation : orientation,
                            barrelPos,
                            ...
                            10.0F,
                            color,
                            TileEntityBaseRenderer.ALIGNCENTER);
                }
            }
                ...
                for (ForgeDirection forgeSide : ForgeDirection.VALID_DIRECTIONS) {
                    ...
                    if (barrelEntity.sideUpgrades[forgeSide.ordinal()] == UpgradeSide.REDSTONE) {
                        ...
                        if (barrelEntity.coreUpgrades.levelStructural > 0) {
                            this.renderIconOnBlock(
                                    StructuralLevel.LEVELS[barrelEntity.coreUpgrades.levelStructural].clientData
                                            .getIconItem(),
                                    1,
                                    forgeSide,
                                    isTopBottom ? rotation : orientation,
                                    ...
                                    -0.001F);
                            this.renderTextOnBlock(
                                    "x" + String.valueOf(barrelEntity.coreUpgrades.levelStructural),
                                    forgeSide,
                                    isTopBottom ? rotation : orientation,
                                    barrelPos,
                                    2.0F,
                            ...
                            if (barrelEntity.coreUpgrades.getFreeSlots() > 0) {
                                ...
                                if (freeSlots.length() < 4) this.renderTextOnBlock(
                                        ...
                                        127.0F,
                                        color,
                                        TileEntityBaseRenderer.ALIGNRIGHT);
                                else this.renderTextOnBlock(
                                        freeSlots,
                                        forgeSide,
                                        isTopBottom ? rotation : orientation,
                                        barrelPos,
                        ...
                        if (barrelEntity.coreUpgrades.nStorageUpg > 0) {
                            this.renderStackOnBlock(
                                    TileEntityBarrelRenderer.coreStorage,
                                    forgeSide,
                                    isTopBottom ? rotation : orientation,
                                    barrelPos,
                                    2.0F,
                                    ...
                                    0);
                            this.renderTextOnBlock(
                                    String.valueOf(barrelEntity.coreUpgrades.nStorageUpg) + "x",
                                    forgeSide,
                                    isTopBottom ? rotation : orientation,
                                    barrelPos,
                                    2.0F,
                        ...
                        if (barrelEntity.coreUpgrades.hasRedstone) {
                            this.renderStackOnBlock(
                                    TileEntityBarrelRenderer.coreRedstone,
                                    forgeSide,
                                    isTopBottom ? rotation : orientation,
                                    barrelPos,
                                    2.0F,
                        ...
                        if (barrelEntity.coreUpgrades.hasHopper) {
                            this.renderStackOnBlock(
                                    TileEntityBarrelRenderer.coreHopper,
                                    forgeSide,
                                    isTopBottom ? rotation : orientation,
                                    barrelPos,
                                    2.0F,
                        ...
                        if (barrelEntity.coreUpgrades.hasEnder) {
                            this.renderStackOnBlock(
                                    TileEntityBarrelRenderer.coreEnder,
                                    forgeSide,
                                    isTopBottom ? rotation : orientation,
                                    barrelPos,
                                    2.0F,
                        ...
                        if (barrelEntity.coreUpgrades.hasVoid) {
                            this.renderStackOnBlock(
                                    TileEntityBarrelRenderer.coreVoid,
                                    forgeSide,
                                    isTopBottom ? rotation : orientation,
                                    barrelPos,
                                    2.0F,
                        ...
                        if (barrelEntity.coreUpgrades.hasCreative) {
                            this.renderStackOnBlock(
                                    TileEntityBarrelRenderer.coreCreative,
                                    forgeSide,
                                    isTopBottom ? rotation : orientation,
                                    barrelPos,
                                    2.0F,
    ...
    protected String getBarrelString(TileEntityBarrel barrel) {
        ...
        // int amount = Math.min(barrel.storage.getAmount(), (int)Math.pow(2, barrel.upgradeCapacity) *
        // barrel.storage.getBaseStacks() * barrel.storage.getItem().stackSize);
