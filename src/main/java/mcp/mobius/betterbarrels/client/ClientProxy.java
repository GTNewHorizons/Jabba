...
public class ClientProxy extends BaseProxy {
    ...
    public void registerRenderers() {
        ...
        try {
            ...
            ClientProxy.blockRenderers = (Map<Integer, ISimpleBlockRenderingHandler>) blockRendererField
        ...
        // ClientRegistry.bindTileEntitySpecialRenderer(TileEntityBarrelShelf.class, new
        // TileEntityBarrelShelfRenderer());
    ...
    public void checkRenderers() {
        ...
        if (!(renderer instanceof BlockBarrelRenderer)) {
            throw new RuntimeException(
                    String.format(
                            "Wrong renderer found ! %s found while looking up the Jabba Barrel renderer.",
                            renderer.getClass().getCanonicalName()));
        }
    }
        ...
        ((IReloadableResourceManager) Minecraft.getMinecraft().getResourceManager())
                .registerReloadListener(new IResourceManagerReloadListener() {
                    ...
                    public void onResourceManagerReload(IResourceManager resourcemanager) {
                        // FML forces a ResourcePack reload after MC has finished initialising, allowing for mod icons
                        // to be registered at any point in init
                        // we only want to run on the second and later reloads so that we're reasonably sure all icons
                            ...
                            for (StructuralLevel level : StructuralLevel.LEVELS) {
                                ...
                                StringTranslate.inject(
                                        new ByteArrayInputStream(
                                                ("item.upgrade.structural." + String.valueOf(level.levelNum)
                                                        + ".name="
                                                        + StatCollector.translateToLocal("item.upgrade.structural")
                                                        + " "
                                                        + Utils.romanNumeral(level.levelNum)
                                                        + " ("
                                                        + level.clientData.getMaterialName()
            ...
            if (overrideColorData.length % 2 == 0) {
                for (int i = 0; i < overrideColorData.length; i += 2) {
                    ...
                    if (overrideColorData[i] > 0 && overrideColorData[i] < StructuralLevel.LEVELS.length) {
                        StructuralLevel.LEVELS[overrideColorData[i]].clientData
                                .setColorOverride((0xFF << 24) | overrideColorData[i + 1]);
                    } else {
                        BetterBarrels.log.warn(
                                "Attempting to override the structural tier color for non existant tier: "
                                        + overrideColorData[i]);
                    }
                }
            } else {
