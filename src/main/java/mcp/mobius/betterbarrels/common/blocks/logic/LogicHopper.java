...
public enum LogicHopper {
        ...
        for (ForgeDirection side : ForgeDirection.VALID_DIRECTIONS) {
            if (barrel.sideUpgrades[side.ordinal()] == UpgradeSide.HOPPER) {
                ...
                if ((barrel.sideMetadata[side.ordinal()] & 1) == UpgradeSide.RS_FULL
                        && (!store.hasItem() || store.getAmount() <= 0)) {
                    continue;
                }
                if ((barrel.sideMetadata[side.ordinal()] & 1) == UpgradeSide.RS_EMPT
                        && (store.getAmount() >= store.getMaxStoredCount())) {
                    continue;
                }

                TileEntity targetEntity = barrel.getWorldObj().getTileEntity(
                        barrel.xCoord + side.offsetX,
                        barrel.yCoord + side.offsetY,
                        barrel.zCoord + side.offsetZ);
                if (isStorage(targetEntity)) {
                    if ((barrel.sideMetadata[side.ordinal()] & 1) == UpgradeSide.RS_FULL) {
                        // Output mode

                        int free;
                        if ((free = this.freeSpaceForStack(
                                targetEntity,
                                side.getOpposite(),
                                store,
                                barrel.sideMetadata[side.ordinal()] >> 1)) > 0
                                && this.pushItemToInventory(
                                        targetEntity,
                                        side.getOpposite(),
                                        store,
                                        Math.min(barrel.sideMetadata[side.ordinal()] >> 1, free))) {
                            store.markDirty();
                            transaction = true;
                        ...
                        // Input mode
                        ItemStack pulledStack = pullMatchingItemFromInventory(
                                targetEntity,
                                side.getOpposite(),
                                store,
                                barrel.sideMetadata[side.ordinal()] >> 1);
                        if (pulledStack != null) {
                            if (store.hasItem()) {
    ...
    private boolean pushItemToInventory(TileEntity target, ForgeDirection side, IBarrelStorage barrel,
            int maxTransfer) {
        ItemStack stack = barrel.getStackInSlot(1);
        ...
        if (isMFRLoaded() && target instanceof IDeepStorageUnit) {
            ...
            for (int slot : islots) {
                ...
                if (targetStack == null) {
                    ...
                    slotTransferAmount = Math
                            .min(slotTransferAmount, (targetStack.getMaxStackSize() - targetStack.stackSize));
    ...
    private ItemStack pullMatchingItemFromInventory(TileEntity source, ForgeDirection side, IBarrelStorage barrel,
            int maxTransfer) {
        if (isMFRLoaded() && source instanceof IDeepStorageUnit) {
            ...
            for (int slot : islots) {
                ...
                if ((testStack != null || stack != null)
                        && !sinv.canExtractItem(slot, (testStack != null ? testStack : stack), side.ordinal())) {
                    continue;
                }
                if (stack != null && barrel.sameItem(stack) && stack.stackSize > 0) {
